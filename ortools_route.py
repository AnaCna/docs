...
def build_test_data():
    ...
    for e_id, (u, v, Lb) in enumerate(undirected_edges):
        ...
        for k in range(Lb):
            ...
            if e_id == 2:
                if 10 <= k < 20:
                    dt = datetime(2025, 10, 19, 0, 0, 0)
                if k >= 50:
                    dt = datetime(2025, 10, 21, 0, 0, 0)
...
def build_nodes_and_arcs(data, merged):
    ...
    for e_id, (u, v, _) in enumerate(undirected_edges):
        ...
        for b in blks:
            ...
            if st_nd == start_node:
                startable.add(nid_p)
            if st_nd2 == start_node:
                startable.add(nid_m)
            if (ed_nd is not None) and (ed_nd in end_cands):
                endable.add(nid_p)
            if (ed_nd2 is not None) and (ed_nd2 in end_cands):
                endable.add(nid_m)
    ...
    for e_id, (u, v, _) in enumerate(undirected_edges):
        ...
        for k in range(B):
            ...
            if k == 0:
                endpoint = u
                for e2 in adj_edges_by_node[endpoint]:
                    if e2 == e_id:
                        continue
                    B2 = len(merged[e2])
                    candidates = (
                    ...
                    for cand in candidates:
                        # 只有当该服务块的“起点”与当前端点一致时才能连过去，
                        # 避免跨边时被错误地传送到另一端。
                        if NODES[cand]["start_node"] == endpoint:
                            add_arc(wp, cand, False)
                            add_arc(wm, cand, False)
            if k == B - 1:
                endpoint = v
                for e2 in adj_edges_by_node[endpoint]:
                    if e2 == e_id:
...
def solve_with_cuts(data, topo):
    ...
    SUCC = topo["SUCC"]
    svc_of = topo["svc_of"]
    startable = topo["startable"]
    endable = topo["endable"]
    merged = topo["merged"]
    ...
    for nid in range(N):
        if NODES[nid]["kind"] != "svc":
            model.Add(start_flag[nid] == 0)
            model.Add(end_flag[nid] == 0)
        else:
            if nid not in startable:
                model.Add(start_flag[nid] == 0)
            if nid not in endable:
                model.Add(end_flag[nid] == 0)
    ...
    def extract():
        ...
        start_task = None
        for nid in range(N):
            if not selv[nid] or NODES[nid]["kind"] != "svc":
                continue
            in_cnt = sum(solver.Value(x[(u, nid)]) for (u, v) in x if v == nid)
            if in_cnt == 0:
                start_task = nid
                break
    ...
    sol = {
        ...
        "sel": {
            nd["nid"]: solver.Value((s[nd["nid"]] if nd["kind"] == "svc" else wsel[nd["nid"]]))
            for nd in NODES
        },
...
def print_solution(sol):
    ...
    for i, nid in enumerate(route[:60], 1):
        ...
        if nd["kind"] == "svc":
            print(
                f"{i:03d} 服务  边{nd['edge']}[{edge_names[nd['edge']]}]  块{nd['blk']:02d}{'(+)' if nd['dir']==1 else '(-)'}"
                f"  长{nd['len_m']}m  开工{st}  完工{ed}"
            )
        else:
            print(
                f"{i:03d} 返走  边{nd['edge']}  块{nd['blk']:02d}  长{nd['len_m']}m  开始{st}  结束{ed}"
            )
